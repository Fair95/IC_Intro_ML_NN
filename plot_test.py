...
class Regressor(nn.Module):
    ...
    def score(self, x, y):
        ...
        #Square Root score
        try:
            score = mean_squared_error(Y,result)
        except:
            score = -1.0
                        ...
                        for acti in activation:
                            for lr in learning_rate:
                                for L in L2:
                                    ...
                                    regressor.fit(x_train,y_train,x_valid,y_valid)
                                    #error = regressor.score(x_valid, y_valid)
                                    plt.plot(regressor.val_loss,label = 'validation loss')
                                    plt.plot(regressor.train_loss,label = 'train loss')
                                    plt.title('Learning Curves with Train and Validation Data')
                                    plt.xlabel('Epochs')
                                    plt.ylabel('Loss')
                                    plt.legend()
                                    plt.show()
...
def example_main():
    ...
    train_num = round(4*data.shape[0] / 5)

    # rng = default_rng(seed=1024)
    # shuffle_index = rng.permutation(len(data))
    # data = data.iloc[shuffle_index]
    # data.reset_index(drop=True, inplace=True)
    # train_num = round(4*data.shape[0] / 5)
    # data_train = data.loc[:train_num,:]
    # data_valid = data.loc[train_num+1:,:]
    # data_train.to_csv('train.csv',index=False)
    # data_valid.to_csv('valid.csv',index=False)
    # # Training
